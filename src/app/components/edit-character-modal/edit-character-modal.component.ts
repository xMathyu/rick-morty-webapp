...
@Component({
  ...
  template: `
    <!-- Overlay layer (transparent background to view main page) -->
    <div
    ...
    >
      ...
      >
        <h3 class="text-xl font-bold text-gray-800 mb-4">Edit Character</h3>
        <form #editForm="ngForm" class="space-y-4">
          <!-- Name -->
          <div>
        ...
        <div class="mt-6 flex justify-end space-x-4">
          ...
          <button
            ...
            [attr.aria-label]="'Confirm editing'"
          >
            Save
          </button>
        </div>
...
export class EditCharacterModalComponent implements OnInit {
  ...
  @Output() edit = new EventEmitter<Partial<Character>>();
  ...
  ngOnInit(): void {
    this.originSearch = this.character.origin ? this.character.origin.name : '';
    this.locationSearch = this.character.location
      ? this.character.location.name
      : '';

    this.characterService.getLocations().subscribe(
      (res) => {
        const rmLocations = (res.results as RMLocation[] | undefined) ?? [];
        this.locations = rmLocations.map((loc) => ({
          name: loc.name,
          url: loc.url,
        }));
      },
      (error: any) => console.error('Error fetching locations:', error)
    );
  }
